            ...
            >
                ...
                <div className="p-4 border-t border-slate-100">
                    <button
                        ...
                        className="flex items-center w-full h-10 px-3 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all group"
                    >
                                ...
                                <div className="text-left">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest leading-none">
                                            {planStats.jobsRemaining} Jobs Left
                                        </p>
                                    </div>
                                        ...
                                        >
                                            ...
                                            <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                                                {notifications.length === 0 ? (
                                                    <div className="p-10 text-center text-slate-400 opacity-50">
                                                    ...
                                                    notifications.map((n) => (
                                                        <div 
                                                            ...
                                                            onClick={() => {
                                                                markAsRead(n.id);
                                                                if (n.type.includes('JOB')) navigate('/recruiter/jobs');
                                                                if (n.type === 'NEW_MESSAGE') navigate('/recruiter/messages');
                                                                setShowNotifications(false);
                                                            }}
                                                            className={`p-4 border-b border-slate-50 last:border-0 cursor-pointer hover:bg-slate-50 transition-colors flex gap-3 ${!n.isRead ? 'bg-indigo-50/30' : ''}`}
                                                        >
                                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${!n.isRead ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'}`}>
                                                                <Clock size={14}/>
                                                            </div>
                        ...
                        <div className="relative border-l border-slate-200 pl-6">
                            ...
                            >
                                <div className="text-right hidden sm:block">
                                   ...
                                   <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${company?.isApproved ? 'text-emerald-500' : 'text-slate-400'}`}>
                                       {company?.isApproved ? 'Verified Partner' : 'Verification Pending'}
                                   </p>
                                </div>                                <div className="w-10 h-10 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-center overflow-hidden shadow-sm group-hover:border-indigo-300 transition-all">
                                ...
                                {showProfileDropdown && (
                                    <>
                                        ...
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0, scale: 1 }}
                                            exit={{ opacity: 0, y: 10, scale: 0.95 }}
                                            className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 py-2 overflow-hidden"
                                        >
                                            <Link to="/recruiter/profile" onClick={() => setShowProfileDropdown(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-colors">
                                                <User size={16} /> My Profile
                                            </Link>
                                            <Link to="/recruiter/settings" onClick={() => setShowProfileDropdown(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-colors border-b border-slate-50">
                                                <Settings size={16} /> Settings
                                            </Link>
                                            <button onClick={logoutHandler} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-rose-600 hover:bg-rose-50 transition-colors text-left">
